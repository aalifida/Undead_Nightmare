...
namespace scgFullBodyController
{
    ...
    public class GunController : MonoBehaviour
    {
        ...
        [Header("Ammo")]
        public int startingTotalBullets = 250;
        public int totalAmmoIncreaseAmount = 10;
        void OnEnable()
        {
        ...
        void Start()
        {
            ...
            scopeButton.onClick.AddListener(OpenScope);
        ...
        void Update()
        {
            if (firingCheck && !firing && reloading == false && bulletsInMag > 0 && !cycling && !swapping)
            {
                firing = true;
            ...
            else if (!firingCheck || bulletsInMag == 0)
            {
                firing = false;
                recoilSemi = false;
                recoilAuto = false;
            ...
            if (ReloadCheck && !reloading && !firing && bulletsInMag < bulletsPerMag && totalBullets > 0)
            {
        ...
        {
            totalBullets += totalAmmoIncreaseAmount;
        ...
        void LateUpdate()
        {
            if (scopeCheck && aimFinished && !swapping)
            {
            ...
            else if (!scopeCheck && aiming && !swapping)
            {
        ...
        {
            int sniperUpgradeCost = 10000;

            int currentCoins = PlayerPrefs.GetInt("coins", 0);
            if (Weapon == WeaponTypes.Rifle && currentCoins >= sniperUpgradeCost)
            {
                bulletsPerMag += 2;
                currentCoins -= sniperUpgradeCost;
        ...
        {
            int arUpgradeCost = 15000;

            int currentCoins = PlayerPrefs.GetInt("coins", 0);
            if (Weapon == WeaponTypes.Rifle && currentCoins >= arUpgradeCost)
            {
                bulletsPerMag += 5;
                currentCoins -= arUpgradeCost;
                PlayerPrefs.SetInt("coins", currentCoins);
            }
            else
            {
        ...
        {
            int pistolUpgradeCost = 8000;

            int currentCoins = PlayerPrefs.GetInt("coins", 0);
            if (Weapon == WeaponTypes.Pistol && currentCoins >= pistolUpgradeCost)
            {
                bulletsPerMag += 3;
                currentCoins -= pistolUpgradeCost;
                PlayerPrefs.SetInt("coins", currentCoins);
            }
            else
            {
